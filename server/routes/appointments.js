...
router.get('/citas', async (req, res) => {
    try {
        const result = await pool.query("SELECT * FROM appointments WHERE status != 'cancelled' AND status != 'cancelada' ORDER BY date ASC, time ASC");
...
router.get('/cancelaciones', async (req, res) => {
    try {
        const result = await pool.query("SELECT * FROM appointments WHERE status = 'cancelled' OR status = 'cancelada' ORDER BY deleted_at DESC, date ASC");
        res.json(result.rows);
    } catch (err) {
        console.error('Error ejecutando query:', err);
...
router.put('/citas/:id/status', async (req, res) => {
    ...
    try {
        ...
        let queryText = 'UPDATE appointments SET status = $1 WHERE id = $2 RETURNING *';
        let queryParams = [dbStatus, id];

        if (dbStatus === 'cancelled') {
            queryText = 'UPDATE appointments SET status = $1, deleted_at = NOW() WHERE id = $2 RETURNING *';
        }
