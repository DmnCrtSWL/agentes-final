<template>
  <admin-layout>
    <div class="p-6">
    <h1 class="text-2xl font-bold mb-6 text-gray-900 dark:text-white">Citas Canceladas</h1>
    ...
    <div v-if="selectedAppointment && viewModalOpen" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" @click="closeViewModal">
      <div class="bg-white dark:bg-gray-800 rounded-lg p-6 max-w-2xl w-full mx-4" @click.stop>
        <div class="flex justify-between items-start mb-4">
          <h2 class="text-xl font-bold text-gray-900 dark:text-white">Detalle de Cita Cancelada</h2>
        ...
        <div class="space-y-4">
          ...
          <div v-if="selectedAppointment.deleted_at">
            <h3 class="text-sm font-medium text-gray-500 dark:text-gray-400">Cancelada el</h3>
            <p class="text-gray-900 dark:text-white">{{ formatDate(selectedAppointment.deleted_at) }}</p>
          </div>
        </div>
      </div>
    </div>
...
const fetchAppointments = async () => {
  try {
    loading.value = true;
    error.value = null;
    const response = await fetch(`${API_URL}/api/appointments/cancelaciones`);
    
    if (!response.ok) {
      throw new Error('Error al cargar las citas canceladas');
    }
    ...
    error.value = 'Error al cargar las citas canceladas. Por favor, intenta de nuevo.';
  } finally {
    loading.value = false;
  }
};
...
const cancelAppointment = (id: any) => {
  // This is already cancelled, so this action doesn't make sense
  // But keeping it for consistency with the table structure
